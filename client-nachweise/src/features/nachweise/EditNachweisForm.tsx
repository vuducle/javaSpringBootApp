...
export function EditNachweisForm({
  nachweisId,
  ...
  useEffect(() => {
    const fetchNachweis = async () => {
      try {
        ...
        // Set basic info
        setValue('name', data.azubiName || user.name || '');
        setValue('datumStart', data.datumStart || '');
        setValue('datumEnde', data.datumEnde || '');
        setValue('nummer', data.nummer || 1);
        setValue(
          'ausbildungsjahr',
          String(data.ausbildungsjahr || '')
        );
        setValue('ausbilderId', data.ausbilderId || '');
        setValue('listEvery', data.listEvery || '');
        setValue('remark', data.remark || '');
        setValue('date_Azubi', data.dateAzubi || '');
        setValue('sig_Azubi', data.sigAzubi || '');
        setValue('sig_Ausbilder', data.sigAusbilder || '');
        ...
        console.error('Fehler beim Laden des Nachweises:', error);
        showToast(t('nachweis.errorLoadingNachweis'), 'error');
      } finally {
        setIsFetching(false);
      }
    };
  ...
  const onSubmit = async (data: PdfGenerationFormValues) => {
    ...
    try {
      ...
      // PUT request to update the Nachweis
      await api.put(`/api/nachweise/${nachweisId}`, {
      ...
      showToast(t('nachweis.updateSuccessMessage'), 'success');

      // Redirect to nachweise overview
      router.push('/nachweise-anschauen');
    } catch (error: unknown) {
      if (error instanceof Error) {
        showToast(error.message, 'error');
      } else {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Lade Nachweis...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      <Card className="w-full max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle>{t('nachweis.editTitle')}</CardTitle>
          <CardDescription>
            {t('nachweis.editDescription')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form
          ...
          >
            ...
            <div className="space-y-4 p-4 border rounded-lg">
              ...
              <div className="space-y-2">
                <Label htmlFor="remark">{t('nachweis.remark')}</Label>
